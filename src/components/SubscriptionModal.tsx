...
export function SubscriptionModal({open, onOpenChange, currentPlan, onSubscribe, onManage}: SubscriptionModalProps) {
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className='w-[calc(100vw-2rem)] sm:max-w-5xl max-h-[85vh] overflow-y-auto'>
				<DialogHeader>
					...
					<DialogDescription className='text-base'>Elige el plan que mejor se adapte a tus necesidades culinarias</DialogDescription>
					{!detectingCountry && paymentAvailable && (
						<div className='flex items-center gap-2 pt-2'>
							...
							<Badge variant='secondary'>{currency === 'ARS' ? 'Precios en Pesos Argentinos' : 'Precios en USD'}</Badge>
						</div>
					)}
					{!detectingCountry && !paymentAvailable && (
						<div className='flex items-center gap-2 pt-2'>
				...
				{showEmailStep && selectedPlan && (
					<motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} className='py-6'>
						<div className='max-w-md mx-auto space-y-6'>
							<Button variant='ghost' size='sm' onClick={handleBackFromEmail} className='mb-2'>
							...
							<div className='text-center space-y-2'>
								...
								<p className='text-muted-foreground'>
									Ingresá el email de tu cuenta de MercadoPago para completar la suscripción al plan{' '}
									{selectedPlan === 'weekly' ? 'Semanal' : 'Mensual'}.
								</p>
							</div>
							...
							<Alert>
								...
								<AlertDescription>
									Usamos este email para vincular tu suscripción con tu cuenta de MercadoPago. Asegurate de que sea el email
									correcto.
								</AlertDescription>
							</Alert>
						</div>
				...
				<AnimatePresence>
					{showSuccess && !showEmailStep && (
						<motion.div initial={{opacity: 0, y: -10}} animate={{opacity: 1, y: 0}} exit={{opacity: 0, y: -10}}>
							<Alert className='bg-primary/10 border-primary/30'>
								...
								<AlertDescription>¡Bienvenido al plan premium! Tu suscripción está activa.</AlertDescription>
							</Alert>
						</motion.div>
					)}
					{showCanceled && !showEmailStep && (
						<motion.div initial={{opacity: 0, y: -10}} animate={{opacity: 1, y: 0}} exit={{opacity: 0, y: -10}}>
							<Alert>
								<AlertDescription>No te preocupes, puedes suscribirte cuando quieras.</AlertDescription>
							</Alert>
						</motion.div>
					)}
				</AnimatePresence>
							...
							{!detectingCountry && !paymentAvailable && (
								...
								>
									<div className='flex items-center gap-3'>
										...
										<div>
											<p className='font-semibold text-foreground'>Próximamente en tu país 🌎</p>
											<p className='text-sm text-muted-foreground'>
												Por ahora los pagos están disponibles solo en Argentina. ¡Estamos creciendo y pronto llegaremos a más
												países!
											</p>
										</div>
									</div>
							...
							{currentPlan === 'free' && !detectingCountry && paymentAvailable && (
								...
								>
									<div className='flex items-center gap-3'>
										...
										<div>
											<p className='font-semibold text-foreground'>¡Ahorra {formatSavings(savings)} por mes!</p>
											<p className='text-sm text-muted-foreground'>
												...
												<CardHeader className='text-center pb-4'>
													<div
														className={`mx-auto w-16 h-16 rounded-full ${
															plan.popular ? 'bg-gradient-to-br from-primary to-purple-600' : 'bg-primary/10'
						...
						<TabsContent value='comparison' className='mt-6'>
							<Card>
								<CardHeader>
									...
									<CardDescription>Compara todas las características de nuestros planes</CardDescription>
								</CardHeader>
								<CardContent>
									...
									{currentPlan === 'free' && paymentAvailable && (
										...
										>
											...
											<div className='flex gap-3'>
												<Button onClick={() => handleSubscribe('weekly')} disabled={loading !== null} variant='outline'>
													{loading === 'weekly' ? <Loader2 className='h-4 w-4 animate-spin mr-2' /> : null}
													Plan Semanal
												</Button>
												<Button
									...
									{currentPlan === 'free' && !paymentAvailable && (
										...
										>
											<h3 className='text-lg font-semibold mb-2'>Próximamente en tu país 🌎</h3>
											<p className='text-muted-foreground'>
												Por ahora los pagos están disponibles solo en Argentina. ¡Estamos creciendo y pronto llegaremos a más
												países!
											</p>
